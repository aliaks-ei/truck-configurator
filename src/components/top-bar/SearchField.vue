...
export default {
	...
	methods: {
		...mapActions(['readInternalTruckElements', 'readExternalTruckElements']),
		...mapMutations(['updateSearchQuery']),

		searchByQuery: helpers.debouncedMethod(async function searchForTruckElements() {
			try {
				const readInternalElements = this.readInternalTruckElements();
				const readExternalElements = this.readExternalTruckElements();

				await Promise.all([readInternalElements, readExternalElements]);
			} catch {
