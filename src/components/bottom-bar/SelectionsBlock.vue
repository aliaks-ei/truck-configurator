<template>
	<div class="selections-block">
		...
		<div class="selections-block__items">
			<selection
				v-for="truckElem in selectedElements.internalElements"
				:key="truckElem.name"
			...
			<selection
				v-for="truckElem in selectedElements.externalElements"
				:key="truckElem.name"
				:title="capitalizeFirstLetter(truckElem.name)"
		...
		<span
			...
			v-if="selectedTruckElementsLength && !isCombinationsPage"
			@click="unselectAll"
		>
			Unselect all
		</span>
...
import Selection from '@/components/bottom-bar/SelectionsBlockSelection.vue';
...
export default {
	...
	methods: {
		...
		handleCloseIconClick({ elem, isExternal }) {
			...
			!this.selectedTruckElementsLength && this.updateCombinationsPageState();
